  ...
  for (const obj of loadFiles) {
    ...
    router[method]( route_uri, passportRouter,
      ...
      async (req, res, next) => {
        ...
        const log = {
          url: req.url,
          userSeq: args.user ? args.user.seq : null,
          clientSeq: args.client ? args.client.seq : null,
        };
        logger({ type: 'logging', topic: 'request', message: JSON.stringify(log) });
        try {
          // user role
